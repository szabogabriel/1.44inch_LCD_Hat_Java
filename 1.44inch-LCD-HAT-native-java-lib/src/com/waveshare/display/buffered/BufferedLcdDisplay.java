...
public class BufferedLcdDisplay extends LcdDriver implements LcdDisplay {
	...
	private BufferedImage currentBackground =  new BufferedImage(DISPLAY.WIDTH, DISPLAY.HEIGHT, BufferedImage.TYPE_INT_RGB);
	private BufferedImage currentBufferedImage = new BufferedImage(DISPLAY.WIDTH, DISPLAY.HEIGHT, BufferedImage.TYPE_INT_RGB);
	...
	private void createDataWriterThread() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				byte [] data;
				int leftover = (DISPLAY.WIDTH * DISPLAY.HEIGHT * 2) % maxLength;
				byte [][] buffer = new byte [((DISPLAY.WIDTH * DISPLAY.HEIGHT * 2) / maxLength) + (leftover > 0 ? 1 : 0)][maxLength];
				long t1, t2;
				while (true) {
					try {
	...
	private short [] newBuffer() {
		return new short [DISPLAY.WIDTH * DISPLAY.HEIGHT];
	}
